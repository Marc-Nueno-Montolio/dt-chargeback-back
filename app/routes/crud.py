...
def get_all_dgs(
    ...
    Parameters:
        ...
        dt_id: Optional search string to filter DGs by DT ID
    ...
    # Apply dt_id filter if provided
    if dt_id:
        query = query.filter(DG.dt_id.ilike(f"%{dt_id}%"))
...
def get_all_is(
    ...
    Parameters:
        search: Optional search string to filter IS by name
        dt_id: Optional search string to filter IS by DT ID
        db: Database session dependency
    """
    query = db.query(IS)
    
    if search:
        query = query.filter(IS.name.ilike(f"%{search}%"))
        
    if dt_id:
        query = query.filter(IS.dt_id.ilike(f"%{dt_id}%"))
        
    information_systems = query.all()
...
def get_all_hosts(
    ...
    multiple_dgs: Optional[bool] = None,
    db: Session = Depends(get_db)
):
    """
    ...
    Parameters:
        page: Page number for pagination
        limit: Number of items per page
        search: Search string for host name
        dt_id: Search string for DT ID
        ...
        multiple_dgs: Filter for hosts assigned to multiple DGs
        db: Database session dependency
    """
    ...
    # Apply search filter
    if search:
        query = query.filter(Host.name.ilike(f"%{search}%"))
        
    # Apply dt_id filter
    if dt_id:
        query = query.filter(Host.dt_id.ilike(f"%{dt_id}%"))
    ...
    # Apply multiple DGs filter if specified
    if multiple_dgs is not None:
        dg_count_subquery = (
            db.query(Host.id, func.count(DG.id).label('dg_count'))
            .outerjoin(Host.dgs)
            .group_by(Host.id)
            .subquery()
        )
        if multiple_dgs:
            query = query.join(dg_count_subquery, Host.id == dg_count_subquery.c.id)\
                        .filter(dg_count_subquery.c.dg_count > 1)
        else:
            query = query.join(dg_count_subquery, Host.id == dg_count_subquery.c.id)\
                        .filter(dg_count_subquery.c.dg_count <= 1)
...
def get_host_filters(db: Session = Depends(get_db)):
    ...
    try:
        ...
        memory_stats = db.query(
            func.min(Host.memory_gb).label('min_memory'),
            func.max(Host.memory_gb).label('max_memory')
        ).first()
        ...
        return {
            ...
            "memory_range": {
                "min": memory_stats.min_memory if memory_stats.min_memory else 0,
                "max": memory_stats.max_memory if memory_stats.max_memory else 0
            }
        }
    except Exception as e:
...
def get_all_applications(
    ...
    Parameters:
        page: Page number for pagination
        limit: Number of items per page
        search: Search string for application name
        dt_id: Search string for DT ID
    ...
    # Apply dt_id filter
    if dt_id:
        query = query.filter(Application.dt_id.ilike(f"%{dt_id}%"))
...
def get_all_synthetics(
    ...
    try:
        ...
        # Apply dt_id filter
        if dt_id:
            query = query.filter(Synthetic.dt_id.ilike(f"%{dt_id}%"))
